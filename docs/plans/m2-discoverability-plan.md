...
How we make it clear that a company orb is clickable (drill into its galaxy)
without breaking the cosmic minimalism. This is a sub-plan of `m2-plan.md` (the
fly-into-orb drill-in); it covers **only the affordance**, not the fetch,
parser, galaxy layout, or transition. Those live in `m2-plan.md` §5–§9.

## 0. The problem

After a fresh seed, the atlas renders the company chain as glassy orbs with
...
fake affordance here"). Users don't know an orb is a door. M2 makes the click
live (`m2-plan.md` §9: "company orbs now carry a click handler; the dormant
Level-0 styling becomes live"). This plan is that styling.

## 1. Decisions (settled in brainstorm)

- **Zero words.** No "Click to explore" labels, no per-orb tooltips, no ambient
...
- **Three layers, on three timescales:**
  1. **Hover baseline** — immediate per-orb feedback, rehearses the fly-in.
  2. **Post-seed ripple** — a one-time wordless "these are alive" wave, first run.
  3. **Orbiting ring on hover** — the signature, metaphor-native signal: an orb
...
1. **Cursor** over any company orb is a pointer (the one universal "clickable").
2. **Hovering** an orb: it lifts and scales up slightly, its corona blooms
   brighter in its own `--orb-color`, and a **faint ring** (the "this is a
...
3. **Immediately after the seed ignition finishes**, a single gentle pulse
   travels down the chain, left to right (the orbs "breathe once"), then the
...
**Where:** CSS only, on the existing `.career-node` / `.career-orb` /
...
- **Lift + corona bloom.** On `.career-node:hover .career-orb`:
  - a small `transform: scale(~1.06) translateY(-2px)` (the orb rises toward
...
- **Interaction with the ambient `orbfloat`.** `.career-orb` already runs an
  infinite `orbfloat` translateY animation. A hover `transform` on the same
  ...
  inner element, keep `orbfloat` on `.career-orb`), so the two transforms live
...
- The intro already staggers each orb's ignition by `calc(var(--i) * --stagger)`
  and starts the ambient float at `calc(var(--i) * --stagger + var(--pop))`
  (after that orb settles). The ripple is **one more keyframed pulse** slotted at
  that same per-orb settle moment, so it reads as a wave that follows ignition
  down the chain.
- **Mechanism:** a short keyframe (e.g. a ~1.04 scale-and-glow swell and back,
  ...
  `orbfloat` conflict), with `animation-delay` keyed on `--i` so orb 0 pulses
  ...
  it never repeats.
- **Sequencing:** the pulse should begin **as/just after** each orb finishes
  ...
  the overshoot tail of `orb-ignite`.
...
orbiting dots) appears around the orb, previewing that the orb **contains a
system** — the galaxy of people you're about to fly into. The affordance *is* the
concept.

- **Where:** a new decorative child inside `.career-orb` (an `::before`/`::after`
  pseudo-element is cleanest — no new DOM, no React change). The existing
  `.career-orb` is already `position: relative`, so an absolutely-positioned ring
  anchors to it for free.
- **Look:** a thin, low-opacity circular border slightly larger than the orb
  (a hair outside the bright rim), tinted with `--orb-color` so it inherits the
...
- **`orbfloat` composition (§3).** Two transforms on one element conflict; keep
  ambient float and hover/ripple on separate elements.
- **`prefers-reduced-motion`.** The file already has reduced-motion blocks for
  `orbfloat`, the intro, and the cosmos. Add matching guards: hover degrades to a
...
1. Layer 1 (cursor + lift + corona), resolving the `orbfloat` wrapper split
